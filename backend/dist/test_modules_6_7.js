...
async function runModules6And7Tests() {
    ...
    try {
        ...
        try {
            await itineraryBuilderService_js_1.ItineraryBuilderService.createSection(demoUser.id, testTrip.id, {
        ...
        const sec1 = await itineraryBuilderService_js_1.ItineraryBuilderService.createSection(demoUser.id, testTrip.id, {
        ...
        const sec2 = await itineraryBuilderService_js_1.ItineraryBuilderService.createSection(demoUser.id, testTrip.id, {
        ...
        const updatedSec1 = await itineraryBuilderService_js_1.ItineraryBuilderService.updateSection(demoUser.id, testTrip.id, sec1.id, {
        ...
        const item1 = await itineraryBuilderService_js_1.ItineraryBuilderService.addItemToSection(demoUser.id, testTrip.id, sec1.id, {
        ...
        const item2 = await itineraryBuilderService_js_1.ItineraryBuilderService.addItemToSection(demoUser.id, testTrip.id, sec1.id, {
        ...
        const reordered = await itineraryBuilderService_js_1.ItineraryBuilderService.reorderItems(demoUser.id, testTrip.id, sec1.id, [item2.id, item1.id]);
        ...
        const daywise = await itineraryViewService_js_1.ItineraryViewService.getDayWiseView(demoUser.id, testTrip.id);
        ...
        const timeline = await itineraryViewService_js_1.ItineraryViewService.getTimelineView(demoUser.id, testTrip.id);
        assert(timeline.events.length >= 4 && timeline.events.some((e) => e.type === 'ARRIVAL') && timeline.events.some((e) => e.type === 'MEAL'), 'getTimelineView returns chronological events including destination arrivals and meals');
        // 7. Test Summary View Endpoint Logic
        const summary = await itineraryViewService_js_1.ItineraryViewService.getSummaryView(demoUser.id, testTrip.id);
        assert(summary.totalDurationDays === 10 && summary.totalActivitiesCount === 2 && summary.destinationSequence.length === 2, 'getSummaryView calculates total duration, activity count, and destination sequence');
        // 8. Test Item Removal & Section Deletion
        await itineraryBuilderService_js_1.ItineraryBuilderService.removeItem(demoUser.id, testTrip.id, item1.id);
        const item1Deleted = await db_js_1.prisma.itineraryItem.findUnique({ where: { id: item1.id } });
        assert(item1Deleted === null, 'removeItem deletes single item from database');
        await itineraryBuilderService_js_1.ItineraryBuilderService.deleteSection(demoUser.id, testTrip.id, sec2.id);
        const sec2Deleted = await db_js_1.prisma.itinerarySection.findUnique({ where: { id: sec2.id } });
