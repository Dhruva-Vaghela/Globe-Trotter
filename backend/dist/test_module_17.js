...
async function runModule17Tests() {
    ...
    try {
        ...
        assert(uploadRes.success && !!uploadRes.imageUrl && uploadRes.filename.endsWith('.png'), 'UploadService successfully parses image payload and generates static URL');
        // 2. Test Global Search Service
        const searchRes = await searchService_js_1.SearchService.globalSearch('Goa');
        assert(Array.isArray(searchRes.trips) &&
            Array.isArray(searchRes.destinations) &&
            Array.isArray(searchRes.activities) &&
            Array.isArray(searchRes.posts), 'SearchService performs unified global search across trips, destinations, activities, and posts');
        console.log('\n========================================================================');
