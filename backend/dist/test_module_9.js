...
async function runModule9Tests() {
    ...
    try {
        ...
        const overview = await calendarService_js_1.CalendarService.getCalendarOverview(demoUser.id);
        assert(typeof overview.tripsCount === 'number' &&
            typeof overview.totalEventsCount === 'number' &&
            Array.isArray(overview.events), 'getCalendarOverview() returns transformed calendar events array and trip counts');
        ...
        const tripData = await calendarService_js_1.CalendarService.getTripCalendarData(demoUser.id, demoTrip.id);
        assert(tripData && Array.isArray(tripData.events), 'getTripCalendarData() returns calendar data for specific trip');
        console.log('\n===========================================================');
