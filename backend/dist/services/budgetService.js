...
class BudgetService {
    ...
    static async setTripBudget(userId, tripId, totalBudget) {
        await this.getTripAndVerifyOwner(userId, tripId);
        if (totalBudget < 0) {
            throw new AppError_js_1.AppError('Total budget cannot be negative', 400);
        }
        return await db_js_1.prisma.budget.upsert({
            where: { tripId },
            create: {
                tripId,
                totalBudget,
    ...
    static async getBudgetOverview(userId, tripId) {
    ...
    static async listExpenses(userId, tripId, category) {
        await this.getTripAndVerifyOwner(userId, tripId);
        return await db_js_1.prisma.expense.findMany({
            where: {
                tripId,
                ...(category && { category }),
            },
    ...
    static async addExpense(userId, tripId, input) {
        await this.getTripAndVerifyOwner(userId, tripId);
        if (input.amount < 0) {
            throw new AppError_js_1.AppError('Expense amount cannot be negative', 400);
        }
        if (!input.description.trim()) {
            throw new AppError_js_1.AppError('Description is required', 400);
        }
        return await db_js_1.prisma.expense.create({
            data: {
                tripId,
                category: input.category || client_1.ExpenseCategory.MISCELLANEOUS,
                amount: input.amount,
    ...
    static async updateExpense(userId, tripId, expenseId, input) {
        await this.getTripAndVerifyOwner(userId, tripId);
        const existingExpense = await db_js_1.prisma.expense.findFirst({
            where: { id: expenseId, tripId },
        });
        if (!existingExpense) {
            throw new AppError_js_1.AppError('Expense record not found', 404);
        }
        if (input.amount !== undefined && input.amount < 0) {
            throw new AppError_js_1.AppError('Expense amount cannot be negative', 400);
        }
        return await db_js_1.prisma.expense.update({
            where: { id: expenseId },
            data: {
    ...
    static async deleteExpense(userId, tripId, expenseId) {
        ...
        await db_js_1.prisma.expense.delete({
            where: { id: expenseId },
        });
