...
async function runModule11Tests() {
    ...
    try {
        ...
        let ownerUser = await db_js_1.prisma.user.findFirst({ where: { email: 'owner@globetrotter.app' } });
        if (!ownerUser) {
            ownerUser = await db_js_1.prisma.user.create({
                data: {
        ...
        let visitorUser = await db_js_1.prisma.user.findFirst({ where: { email: 'visitor@globetrotter.app' } });
        if (!visitorUser) {
            visitorUser = await db_js_1.prisma.user.create({
                data: {
        ...
        const sourceTrip = await db_js_1.prisma.trip.create({
            data: {
                userId: ownerUser.id,
        ...
        const shareResult = await publicItineraryService_js_1.PublicItineraryService.generateShareLink(ownerUser.id, sourceTrip.id);
        assert(!!shareResult.shareToken && shareResult.shareUrl.includes('/share/'), 'generateShareLink creates active public token and URL');
        const tripAfterShare = await db_js_1.prisma.trip.findUnique({ where: { id: sourceTrip.id } });
        ...
        const publicData = await publicItineraryService_js_1.PublicItineraryService.getPublicItineraryByToken(shareResult.shareToken);
        assert(publicData.owner.name === 'Owner Travel Enthusiast' &&
            publicData.trip.name === 'Shared Tokyo & Kyoto Adventure' &&
            publicData.sections.length === 1 &&
            publicData.sections[0].items[0].title === 'Tsukiji Outer Market Breakfast', 'getPublicItineraryByToken retrieves owner info, metadata, stops, sections, and items');
        // 3. Test Get Public Itinerary by Trip ID
        const publicDataById = await publicItineraryService_js_1.PublicItineraryService.getPublicItineraryById(sourceTrip.id);
        ...
        const copyResult = await publicItineraryService_js_1.PublicItineraryService.copyPublicTrip(visitorUser.id, shareResult.shareToken);
        assert(copyResult.success && !!copyResult.clonedTripId, 'copyPublicTrip clones public trip into visitor workspace');
        const clonedTrip = await db_js_1.prisma.trip.findUnique({
            where: { id: copyResult.clonedTripId },
            include: { stops: true, sections: { include: { items: true } }, budget: true },
        });
        assert(clonedTrip?.userId === visitorUser.id &&
            ...
            clonedTrip.budget?.totalBudget === 120000, 'Cloned trip preserves trip stops, sections, items, and budget under new owner');
        // 5. Test Revoke Share Link
        await publicItineraryService_js_1.PublicItineraryService.revokeShareLink(ownerUser.id, sourceTrip.id);
        let revokedAccessDenied = false;
        try {
            await publicItineraryService_js_1.PublicItineraryService.getPublicItineraryByToken(shareResult.shareToken);
        }
        catch (err) {
        ...
        // Cleanup test trips
        await db_js_1.prisma.trip.delete({ where: { id: sourceTrip.id } });
        await db_js_1.prisma.trip.delete({ where: { id: copyResult.clonedTripId } });
        console.log('\n========================================================================');
