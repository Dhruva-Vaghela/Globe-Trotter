...
async function testLiveHttpApi() {
  ...
  try {
    ...
    assert(
      res1.status === 200 && json1.success === true && Array.isArray(json1.data) && json1.data.length >= 30,
      `GET /api/v1/activities returned status 200 with ${json1.data?.length} activities`
    );
    ...
    assert(
      res2.status === 200 && json2.success === true && json2.data.length > 0,
      `GET /api/v1/activities?search=Taj&maxCost=5000 returned status 200 with filtered results`
    );
    ...
    assert(
      res3.status === 200 &&
        ...
        !!json3.data.locationMap,
      `GET /api/v1/activities/:id returned status 200 with durationBreakdown and locationMap`
    );
    ...
    assert(
      ...
      `POST /api/v1/auth/login returned status 200 and valid JWT authentication token`
    );
    ...
    if (!sectionId) {
      const secInDb = await prisma.itinerarySection.findFirst({ where: { tripId: demoTrip.id } });
      if (secInDb) {
        sectionId = secInDb.id;
      } else {
        const createdSec = await prisma.itinerarySection.create({
          data: {
            tripId: demoTrip.id,
    ...
    assert(
      addItemRes.status === 201 && addItemJson.success === true && !!newItemId,
      `POST /api/v1/trips/:tripId/sections/:sectionId/items created ItineraryItem (status 201)`
    );
    ...
    assert(
      ...
      `PUT /api/v1/trips/:tripId/sections/:sectionId/items/reorder reordered items (status 200)`
    );
    ...
    assert(
      ...
      `DELETE /api/v1/trips/:tripId/items/:itemId deleted item (status 200)`
    );
