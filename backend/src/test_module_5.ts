...
async function runModule5Tests() {
  ...
  try {
    ...
    const searchResult = await ActivityService.listActivities({ search: 'Scuba' });
    assert(
      searchResult.length > 0 && searchResult.some((a) => a.name.includes('Scuba')),
      'listActivities({ search: "Scuba" }) returns matching Scuba activity'
    );

    const foodCat = await prisma.activityCategory.findUnique({ where: { name: 'Food & Dining' } });
    if (foodCat) {
      const foodActivities = await ActivityService.listActivities({ category: foodCat.id });
      assert(
        foodActivities.length > 0 && foodActivities.every((a) => a.categoryId === foodCat.id),
        'listActivities({ category: categoryId }) correctly filters by category ID'
      );
    }

    const budgetActivities = await ActivityService.listActivities({ maxCost: 1500 });
    assert(
      budgetActivities.length > 0 && budgetActivities.every((a) => a.estimatedCost <= 1500),
      'listActivities({ maxCost: 1500 }) correctly filters by max cost'
    );

    const sortedActivities = await ActivityService.listActivities({ sortBy: 'cost_asc' });
    let isSorted = true;
    ...
    const details = await ActivityService.getActivityById(targetActivity.id);
    assert(
      details.id === targetActivity.id &&
        !!details.durationBreakdown &&
        !!details.durationBreakdown.formatted &&
        !!details.locationMap,
      'getActivityById(id) returns detailed activity with duration breakdown and location map'
    );
    ...
    if (demoTrip.sections.length === 0) {
      const newSec = await prisma.itinerarySection.create({
        data: {
          tripId: demoTrip.id,
    ...
    const newItem1 = await ActivityService.addActivityToSection(demoUser.id, demoTrip.id, sectionId, {
    ...
    assert(
      newItem1.id !== undefined && newItem1.sectionId === sectionId && newItem1.title === targetActivity.name,
      'addActivityToSection() successfully creates ItineraryItem attached to ItinerarySection'
    );

    const newItem2 = await ActivityService.addActivityToSection(demoUser.id, demoTrip.id, sectionId, {
    ...
    assert(
      newItem2.id !== undefined && newItem2.orderIndex === newItem1.orderIndex + 1,
      'addActivityToSection() auto-increments orderIndex for subsequent items in section'
    );

    // 5. Test PUT /api/v1/trips/:tripId/sections/:sectionId/items/reorder (Reorder itinerary items)
    const reordered = await ActivityService.reorderItineraryItems(demoUser.id, demoTrip.id, sectionId, [
      newItem2.id,
      newItem1.id,
    ]);
    ...
    assert(
      item2After?.orderIndex === 0 && item1After?.orderIndex === 1,
      'reorderItineraryItems() correctly updates item orderIndex positions'
    );

    // 6. Test DELETE /api/v1/trips/:tripId/items/:itemId (Remove activity item)
    await ActivityService.removeItineraryItem(demoUser.id, demoTrip.id, newItem1.id);
    await ActivityService.removeItineraryItem(demoUser.id, demoTrip.id, newItem2.id);

    const checkDeleted = await prisma.itineraryItem.findUnique({ where: { id: newItem1.id } });
